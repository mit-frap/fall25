(** * 6.512 Formal Reasoning About Programs, Fall 2025 - Pset 13 *)

Require Import Frap.
From Stdlib Require Export Eqdep. (* for inj_pair2 as referenced in a hint *)
...
(* Here's a convenient predicate to assert both kinds of stability at once.
   A convenient way to think of it is that P and Q are preserved by R's heap
   modifications. *)
...
Our proof template for logic soundness starts as usual by appealing to an 'invariant weaking' lemma and an 'invariant induction' lemma.  We can also call them the 'progress' lemma and the 'preservation' lemma. Roughly, the first one says that any command with a Hoare triple in the current state is not about to fail, and the second says that if a command with a Hoare triple in the current state takes a step, the resulting command will still have a Hoare triple in the resulting state.

A particularly useful lemma from Pset13Sig that is easily missed is `always_stableP`:
...
Rocq!  It's actually formally independent of Rocq's logic.  However, a
...
`Eqdep` module of the Rocq standard library.
```
Ltac existT_subst :=
